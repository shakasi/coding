...
using System.Linq;
using VNext.Authorization.EntityInfos;
...
namespace VNext.Entity
{
    ...
    public class EntityManager : IEntityManager
    {
        ...
        public virtual Type GetDbContextTypeForEntity(Type entityType)
        {
            ...
            if (dict.Count == 0)
            {
                throw new Exception($"未发现任何数据上下文实体映射配置，请通过对各个实体继承基类“EntityTypeConfigurationBase<TEntity, TKey>”以使实体加载到上下文中");
            }
            ...
            throw new Exception($"无法获取实体类 {entityType} 的所属上下文类型，请通过继承基类“EntityTypeConfigurationBase<TEntity, TKey>”配置实体加载到上下文中");
        }
        ...
        private partial class KeyValueConfiguration : EntityTypeConfigurationBase<KeyValue, Guid>
        {
            /// <summary>
            /// 重写以实现实体类型各个属性的数据库配置
            /// </summary>
