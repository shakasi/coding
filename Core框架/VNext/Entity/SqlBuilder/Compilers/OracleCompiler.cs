...
namespace VNext.Entity
{
    public class OracleCompiler : Compiler
    {
        ...
        protected override SqlResult CompileSelectQuery(SqlBuilder query)
        {
        ...
        protected override string CompileBasicDateCondition(SqlResult ctx, BasicDateCondition condition)
        {
            ...
            switch (condition.Part)
            {
                case "date": // assume YY-MM-DD format
                    if (isDateTime)
                    {
                        valueFormat = $"TO_DATE({value}, 'yyyy-mm-dd hh24:mi:ss')";
                    }
                    else
                    {
                        column = $"TO_DATE('{column}', 'yyyy-mm-dd hh24:mi:ss')";
                        valueFormat = $"TO_DATE({value}, 'yyyy-mm-dd hh24:mi:ss')";
                    }
                    
                    sql = $"{column} {condition.Operator} {valueFormat}";
                    break;
                case "time":
                    if (isDateTime)
                        valueFormat = $"{value}";
