...
namespace VNext.Entity
{
    public abstract partial class BaseQuery<Q>
    {
        ...
        public Q WhereIn(string column, SqlBuilder query)
        {
        ...
        public Q WhereIn(string column, Func<SqlBuilder, SqlBuilder> callback)
        {
            var query = callback.Invoke(new SqlBuilder().SetParent(this));

            return WhereIn(column, query);
        }

        public Q OrWhereIn(string column, SqlBuilder query)
        {
            return Or().WhereIn(column, query);
        }

        public Q OrWhereIn(string column, Func<SqlBuilder, SqlBuilder> callback)
        {
            return Or().WhereIn(column, callback);
        }
        public Q WhereNotIn(string column, SqlBuilder query)
        {
            return Not().WhereIn(column, query);
        }

        public Q WhereNotIn(string column, Func<SqlBuilder, SqlBuilder> callback)
        {
            return Not().WhereIn(column, callback);
        }

        public Q OrWhereNotIn(string column, SqlBuilder query)
        {
            return Or().Not().WhereIn(column, query);
        }

        public Q OrWhereNotIn(string column, Func<SqlBuilder, SqlBuilder> callback)
        {
        ...
        public Q Where(string column, string op, SqlBuilder query)
        {
            return AddComponent("where", new QueryCondition<SqlBuilder>
            {
                Column = column,
                Operator = op,
                Query = query,
        ...
        public Q WhereSub(SqlBuilder query, object value)
        {
            return WhereSub(query, "=", value);
        }

        public Q WhereSub(SqlBuilder query, string op, object value)
        {
            return AddComponent("where", new SubQueryCondition<SqlBuilder>
            {
                Value = value,
                Operator = op,
                Query = query,
        ...
        public Q OrWhereSub(SqlBuilder query, object value)
        {
            return Or().WhereSub(query, value);
        }

        public Q OrWhereSub(SqlBuilder query, string op, object value)
        {
            return Or().WhereSub(query, op, value);
        }

        public Q OrWhere(string column, string op, SqlBuilder query)
        {
            return Or().Where(column, op, query);
        }
        public Q OrWhere(string column, string op, Func<SqlBuilder, SqlBuilder> callback)
        {
            return Or().Where(column, op, callback);
        }

        public Q WhereExists(SqlBuilder query)
        {
        ...
        public Q WhereExists(Func<SqlBuilder, SqlBuilder> callback)
        {
            var childQuery = new SqlBuilder().SetParent(this);
            return WhereExists(callback.Invoke(childQuery));
        }

        public Q WhereNotExists(SqlBuilder query)
        {
            return Not().WhereExists(query);
        }

        public Q WhereNotExists(Func<SqlBuilder, SqlBuilder> callback)
        {
            return Not().WhereExists(callback);
        }

        public Q OrWhereExists(SqlBuilder query)
        {
            return Or().WhereExists(query);
        }
        public Q OrWhereExists(Func<SqlBuilder, SqlBuilder> callback)
        {
            return Or().WhereExists(callback);
        }
        public Q OrWhereNotExists(SqlBuilder query)
        {
            return Or().Not().WhereExists(query);
        }
        public Q OrWhereNotExists(Func<SqlBuilder, SqlBuilder> callback)
        {
